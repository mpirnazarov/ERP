...
<div class="table-responsive">
    <table id="myTable" class="display table table-bordered">
        ...
        <c:forEach items="${projects}" var="project">
            <tr>
                <td><c:out value="${project.projectsByProjectId.code} - ${project.projectsByProjectId.type}"/></td>
                <td><c:out value="${project.projectsByProjectId.projectLocalizationsesById[0].name}"/></td>
                <td><fmt:formatDate value="${project.projectsByProjectId.startDate}" pattern="yyyy MMM dd"/> - <fmt:formatDate value="${project.projectsByProjectId.endDate}" pattern="yyyy MMM dd"/></td>
                <td><c:out value="${project.rolesByProjectId.roleLocalizationsesById[0].name}"/></td>
                <td><fmt:formatDate value="${project.dateFrom}" pattern="yyyy MMM dd"/> - <fmt:formatDate value="${project.dateTo}" pattern="yyyy MMM dd"/></td>
                <td>
                <%
